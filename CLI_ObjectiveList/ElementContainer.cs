...
namespace Cobilas.CLI.ObjectiveList {
    internal class ElementContainer : IDisposable, IReadOnlyList<ElementItem>, IEnumerable<ElementItem> {
        ...
        private static void ElementContainerToElementTag(ElementTag root, ElementItem item, string path) {
            ElementTag element = new ElementTag("element",
                    new ElementAttribute("path", path),
                    new ElementAttribute("status", item.status)
                );
            root.Add(element);

            element.Add(new ElementTag("title", item.title));
            if (!string.IsNullOrEmpty(item.description))
                element.Add(new ElementTag("descriptio", item.description));

            for (int I = 0; I < item.Count; I++)
                ElementContainerToElementTag(element, item[I], $"{path}.{I}");
        }

        private static void ElementTagToElementContainer(ElementTag tag, ElementContainer container) {
            bool noVersion = false;
            tag.ForEach((ElementAttribute E) => {
                if (E.Name == "version")
        ...
        private static void ElementTagToElementContainer1_0(ElementTag tag, ElementContainer container) {
        ...
        private static void ElementTagToElementContainer2_0(ElementTag tag, ElementContainer container) {
            tag.ForEach((ElementTag T) => {
                if (T.Name == "element") {
        ...
        public static explicit operator ElementContainer(ElementTag E) {
            ElementContainer res = new ElementContainer();
            ElementTagToElementContainer(E, res);
            return res;
        }

        public static explicit operator ElementTag(ElementContainer E) {
            ElementTag root = new ElementTag("Root", new ElementAttribute("version", "2.0"));
